...
Here's a brief explanation of what each method in the `CarbonFootprintAgents` class does:

Agents:
- `_create_agents`: Creates all the agents for the class. The agents are:
    - `data_entry_assistant`: Helps users classify emissions, map to scopes, and validate data entries.
    - `report_generator`: Generates human-readable summaries of emissions data.
    - `offset_advisor`: Recommends verified offset options based on user profile and location.
    - `regulation_radar`: Notifies users of upcoming compliance requirements.
    - `emission_optimizer`: Uses historical data to suggest reductions and savings.


Tasks:
- `create_data_entry_task`: Creates a task for the `data_entry_assistant` agent to analyze data and classify it into the appropriate emission scope and category.

- `create_report_summary_task`: Creates a task for the `report_generator` agent to generate a comprehensive summary of emissions data.

- `create_offset_advice_task`: Creates a task for the `offset_advisor` agent to recommend carbon offset options based on an organization's profile.

- `create_regulation_check_task`: Creates a task for the `regulation_radar` agent to analyze regulatory requirements for an organization.

- `create_optimization_task`: Creates a task for the `emission_optimizer` agent to analyze emissions data and identify opportunities for reduction.

Run a crew with each agent to execute their respective tasks.
- `run_data_entry_crew`, `run_report_summary_crew`, `run_offset_advice_crew`, `run_regulation_check_crew`, `run_optimization_crew`:  
A crew consists of an agent and a task. The `verbose` parameter is set to `False` to suppress output.
